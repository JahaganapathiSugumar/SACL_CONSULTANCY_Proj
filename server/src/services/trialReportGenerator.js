...
export const generateAndStoreTrialReport = async (trial_id, trx) => {
    ...
    if (hardRowsForSection.length > 0) {
        ...
        if (visual?.hardness_remarks) {
            doc.font('Helvetica-Oblique').fontSize(7).text(`Remarks: ${visual?.hardness_remarks}`, col1X, hardY, { width: colWidth });
    ...
    if (ndtRowsForSection.length > 0) {
        ...
        if (visual?.ndt_inspection_remarks) {
            doc.font('Helvetica-Oblique').fontSize(7).text(`Remarks: ${visual?.ndt_inspection_remarks}`, col2X, ndtY, { width: colWidth });

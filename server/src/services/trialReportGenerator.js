...
export const fetchTrialData = async (trial_id, trx) => {
    ...
    const [trial_cards] = await trx.query(
        `SELECT tc.*, mc.part_name, mc.pattern_code, mc.chemical_composition AS spec_chem, mc.micro_structure AS spec_micro,
         ...
         FROM trial_cards tc 
         LEFT JOIN master_card mc ON tc.master_card_id = mc.id 
...
export const generateAndStoreTrialReport = async (trial_id, trx) => {
    ...
    // Visual
    if (allCavitiesForSummary.length > 0) {
        const remarksHeight = visual?.remarks ? doc.heightOfString(`Remarks: ${visual.remarks}`, { width: 535 }) + 15 : 15;
    ...
    // Hardness
    if (allCavitiesForSummary.length > 0) {
        const remarksHeight = visual?.hardness_remarks ? doc.heightOfString(`Remarks: ${visual.hardness_remarks}`, { width: 535 }) + 15 : 15;
        ...
        if (visual?.hardness_remarks) {
            ...
            p2NextY += doc.heightOfString(`Remarks: ${visual?.hardness_remarks}`, { width: 535 }) + 2;
        }
    ...
    // NDT
    if (allCavitiesForSummary.length > 0) {
        const remarksHeight = visual?.ndt_inspection_remarks ? doc.heightOfString(`Remarks: ${visual.ndt_inspection_remarks}`, { width: 535 }) + 15 : 15;
        const neededHeight = 30 + allCavitiesForSummary.length * 18 + remarksHeight;
        checkPageSpace(neededHeight);
        ...
        if (visual?.ndt_inspection_remarks) {
            ...
            p2NextY += doc.heightOfString(`Remarks: ${visual?.ndt_inspection_remarks}`, { width: 535 }) + 2;
        }

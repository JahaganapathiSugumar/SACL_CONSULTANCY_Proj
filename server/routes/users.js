...
router.get('/', verifyToken, authorizeRoles('Admin'), asyncErrorHandler(async (req, res, next) => {
  const [rows] = await Pool.execute('SELECT user_id, username, full_name, email, department_id, role, created_at FROM users');
...
router.post('/', verifyToken, authorizeRoles('Admin'), asyncErrorHandler(async (req, res, next) => {
  ...
  const [result] = await Pool.execute(sql, [username, full_name, hash, email, department_id || null, role || null]);

...
export default function AllTrialsPage({ embedded = false }: AllTrialsPageProps) {
    ...
    const totalCols = useMemo(() => {
        ...
        if (user?.role === 'Admin') {
            cols += 2;
        } else if (user?.department_id === 2 || user?.department_id === 3) {
            cols += 1;
        }
        return cols;
    }, [user]);
                        ...
                        <Table stickyHeader size="medium">
                            <TableHead className="premium-table-head">
                                <TableRow>
                                    ...
                                    {(user?.role === 'Admin' || user?.department_id === 2 || user?.department_id === 3) && <TableCell className="premium-table-header-cell" />}
